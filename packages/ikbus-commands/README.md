# @emdzej/ikbus-commands

Per-command codecs (parsers and builders) for BMW I/K-bus frames.  Built on top of [`@emdzej/ikbus-protocol`](../protocol); consumed by [`@emdzej/ikbus-devices`](../devices) and apps.

Each codec is pure: a `parse<X>(message)` function takes an `IKBusMessage` and returns a typed domain object, and a `build<X>(args)` function does the reverse.  Validation throws typed `CommandError` subclasses.
...
```ts
import { encode, decode } from '@emdzej/ikbus-protocol'
import { parseIgnitionStatus, buildIgnitionStatus } from '@emdzej/ikbus-commands'
